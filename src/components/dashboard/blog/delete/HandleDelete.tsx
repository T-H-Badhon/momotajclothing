...
import { handleDeleteArticle } from 'src/components/actions/actionArticles';

const HandleDelete = ({ id }: { id: string }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);

    const handleConfirm = async () => {
        const result = await handleDeleteArticle(id);
        if (result?.success) {
            toast.success('Article delete successfully');
            setIsModalOpen(false);
        } else {
            console.error('Error creating article:', result?.error);
            toast.error('There was an error deleting this article');
        }
    };
